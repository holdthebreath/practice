...
总结，**只要每个线程的操作的执行顺序，满足该线程的程序顺序(即执行顺序符合1->2->3)且执行效果为原子性并对其他线程立即可见，两个线程任意实际执行顺序都是符合顺序一致性的。**
...
补充说明：
- 具体点说，所谓的单线程上下文中线程行为可以完全预测，指的是在单线程上下文中，通过每个读看到的值，预测线程的执行结果，**注意这里并没有谈及看到值的正确性问题，可能是因为在单线程语义下并不存在**。因此程序一定遵守单线程语义(我认为本质的原因是现代cpu一定遵守单线程语义(单核cpu是**顺序的**)，这是由底层决定的)，与其他概念(比如正确同步)不相干，从概念上来说，有点像正确的废话。
