...
具体点说，**线程间操作**1和2读取的值是共享变量，因此具体看到的值应该由JMM确定，而两个线程针对同一个变量的读/写没有**同步操作**，因此这两个操作不存在**synchronized-with**关系，在情况2这样的**执行顺序**下，不存在hb关系**阻止**读看到写的结果(happens-before consistency)。
加粗的全部为概念，可以回忆梳理下。